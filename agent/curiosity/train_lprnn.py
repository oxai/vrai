...
for iteration in range(1000000):
    ...
    for i in range(n_simulation_steps):
        ...
        if evaluating: #if evaulating on hand environment, we use the goal that the environment gives us
            pen_goal = results["desired_goal"]
            goal = np.tile(np.expand_dims(pen_goal,0),(n_steps,1))
            goal = np.reshape(goal.T,(-1))
        if rendering:
    ...
    if not evaluating: #if not evaluating, then train
        # first we add gradients for the goal reconstruction part of the network
        optimizer.zero_grad()
        ...
        if iteration>0: #only do this once we have a previous_lp_value
