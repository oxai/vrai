...
if NOT "%ERRORLEVEL%"=="0" start "" /d "C:\Program Files (x86)\Steam\steamapps\common\NeosVR\" run_neos_vrai_desktop.bat 
tasklist /FI "IMAGENAME eq Neos.exe" 2>NUL | find /I /N "Neos.exe">NUL
if NOT "%ERRORLEVEL%"=="0" timeout /t 50
mlagents-learn eco_npc_config.yaml --run-id=test_eco_demo_new3 --env="..\..\environment\neos\built_env\Unity Environment" --time-scale=2.0 && Taskkill /IM Neos.exe /F
